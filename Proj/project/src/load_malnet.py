...
class MalNetTinyLoader:
    ...
    def load_graphs(self, format: str = "edgelist") -> List[nx.Graph]:
        """
        ...
        if format == "edgelist":
            graphs = self._load_edgelist(graph_dir)
        elif format == "pickle":
    ...
    def _load_edgelist(self, graph_dir: Path) -> List[nx.Graph]:
        ...
        for graph_file in graph_files:
            try:
                G = nx.read_edgelist(str(graph_file), nodetype=int, create_using=nx.Graph())
                graphs.append(G)
